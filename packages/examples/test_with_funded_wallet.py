...
def test_create_studio():
    ...
    try:
        ...
        if balance == 0:
            console.print("  [red]✗[/red] Wallet not funded!")
...
def test_register_agent(sdk, studio_address):
    ...
    try:
        ...
        # Register
        console.print("  [cyan]→[/cyan] Registering agent...")
        agent_id, tx_hash = sdk.chaos_agent.register_agent()
        
        console.print(f"  [green]✓[/green] Agent registered: {agent_id}")
...
def test_register_with_studio(sdk, studio_address, agent_id):
    ...
    try:
        ...
        console.print(f"  [cyan]→[/cyan] Registering with studio...")
        ...
        console.print(f"  [green]✓[/green] Registered with studio")
        console.print(f"  [dim]TX: {tx_hash}[/dim]")
        return True
...
def test_scoring(sdk, studio_address, data_hash):
    ...
    try:
        ...
        console.print(f"  [green]✓[/green] Score committed")
        console.print(f"  [dim]TX: {tx_hash}[/dim]")
        ...
        console.print(f"  [green]✓[/green] Score revealed")
        console.print(f"  [dim]TX: {tx_hash}[/dim]")
...
def test_epoch_closure(sdk, studio_address):
    ...
    try:
        console.print(f"  [cyan]→[/cyan] Closing epoch 1...")
...
def test_rewards(sdk, studio_address):
    ...
    try:
        ...
        console.print(f"  [cyan]→[/cyan] Checking pending rewards...")
        pending = sdk.get_pending_rewards(studio_address)
        console.print(f"  [green]✓[/green] Pending rewards: {pending} wei")
        
        if pending > 0:
            # Withdraw
            console.print(f"  [cyan]→[/cyan] Withdrawing rewards...")
            tx_hash = sdk.withdraw_rewards(studio_address)
            console.print(f"  [green]✓[/green] Rewards withdrawn")
            console.print(f"  [dim]TX: {tx_hash}[/dim]")
        else:
            console.print(f"  [yellow]⚠[/yellow] No rewards to withdraw")
...
def test_reputation(sdk, agent_id):
    ...
    try:
        console.print(f"  [cyan]→[/cyan] Querying reputation for agent {agent_id}...")
        reputation = sdk.chaos_agent.get_reputation(agent_id)
        
        console.print(f"  [green]✓[/green] Reputation entries: {len(reputation)}")
        return True
...
def main():
    ...
    console.print(Panel.fit(
        ...
        "[yellow]⚠ This will execute real on-chain transactions![/yellow]",
